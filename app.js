...
function updateMoveStripActive() {
  ...
  if (current) {
    const centered = current.offsetLeft - (moveStripEl.clientWidth - current.offsetWidth) / 2;
    moveStripEl.scrollTo({ left: Math.max(0, centered), behavior: stripInstant ? 'auto' : 'smooth' });
  }
}
...
(function enableStripControl() {
  const HOLD_MS = 300;        // cuánto hay que mantener para engancharla
  const MOVE_CANCEL = 10;     // moverse antes es un desplazamiento normal
  const EDGE_ZONE = 52;       // franja del extremo que sigue avanzando
  const EDGE_MS = 120;        // ritmo de ese avance continuo

  let holdTimer = null, edgeTimer = null, inertia = null;
  let scrubbing = false, dragging = false;
  let startX = 0, lastX = 0, lastT = 0, velocity = 0;
  let startPly = 0, startScroll = 0, pxPerPly = 56, edgeDir = 0;

  function stopEdge() { clearInterval(edgeTimer); edgeTimer = null; edgeDir = 0; }
  ...
  function beginScrub() {
    ...
    const chip = moveChipEls[currentPly];
    if (chip && chip.offsetWidth) pxPerPly = chip.offsetWidth + 5;
  }

  // Al llegar al extremo el dedo ya no puede seguir, así que la partida
  // continúa sola mientras se mantenga pulsado ahí.
  function updateEdge(x) {
    const r = moveStripEl.getBoundingClientRect();
    let dir = 0;
    if (x > r.right - EDGE_ZONE) dir = 1;
    else if (x < r.left + EDGE_ZONE) dir = -1;
    ...
    edgeTimer = setInterval(() => {
      const total = gameState.positions.length - 1;
      const next = currentPly + edgeDir;
      if (next < 0 || next > total) { stopEdge(); return; }
      goToPly(next);
      startPly = currentPly;   // el recorrido continúa desde aquí
      startX = lastX;
    }, EDGE_MS);
  }
  ...
  moveStripEl.addEventListener('pointerdown', (e) => {
    ...
    velocity = 0;
    startScroll = moveStripEl.scrollLeft;
  ...
  moveStripEl.addEventListener('pointermove', (e) => {
    ...
    if (scrubbing) {
      ...
      if (edgeDir) return;     // en el extremo manda el avance continuo
      const total = gameState.positions.length - 1;
      const target = Math.max(0, Math.min(startPly + Math.round(-dx / pxPerPly), total));
      if (target !== currentPly) goToPly(target);
      return;
    }
    ...
    // La tira acompaña al dedo: se va a la izquierda, contenido a la izquierda.
    if (dragging) moveStripEl.scrollLeft = startScroll - dx;
  });

  function glide() {
    velocity *= 0.94;
    moveStripEl.scrollLeft -= velocity * 16;
    if (Math.abs(velocity) > 0.02) inertia = requestAnimationFrame(glide);
    else inertia = null;
  }
  ...
  moveStripEl.addEventListener('pointercancel', endGesture);
  moveStripEl.addEventListener('pointerleave', endGesture);

  // Tras arrastrar, el dedo no debe además saltar a la jugada que pilló debajo.
...
function openRenameBranchSheet() {
  ...
  branchNameHintEl.textContent = 'Ponle un nombre que te diga de qué va la variante.';
